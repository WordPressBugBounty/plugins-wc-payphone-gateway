...
class Payphone_Gateway_Btn_Process
{
    ...
    private function getDataBillTo($order, $client_tx_id)
    {
        ...
        foreach ($items as $item) {
            ...
            $productos = [
                "productName" => substr(trim(wp_strip_all_tags($item_data['name'])), 0, 50),
                ...
                "productSKU" => substr(trim(wp_strip_all_tags($product->get_sku())), 0, 50),
                "productDescription" => substr(trim(wp_strip_all_tags($product->get_short_description())), 0, 50)
            ];
            $lineItems[] = $productos;
        }
        //DATOS DE ENVIO
        if (!empty($order->get_shipping_method())) {
            $envio = [
                "productName" => substr(trim(wp_strip_all_tags($order->get_shipping_method())), 0, 50),
                ...
                "productSKU" => substr(trim(wp_strip_all_tags("Envio Order :#" . $client_tx_id)), 0, 50),
                "productDescription" => substr(trim(wp_strip_all_tags($order->get_shipping_to_display())), 0, 50)
            ];
            $lineItems[] = $envio;
        }
